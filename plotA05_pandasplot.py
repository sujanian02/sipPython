...
df.cyl.plot(kind='bar')#error becuase no total
df.cyl.value_counts()
df.cyl.value_counts().plot(kind='bar')
df.mpg.plot(kind='hist')
...
df.groupby(['gear','cyl','am']).size().plot(kind='bar', stacked='True')
#gear, cyl, am
my_colors = list(islice(cycle(['b', 'r', 'g', 'y', 'k']), None, len(df)))
df.groupby(['am','vs']).size().plot.bar()
df.groupby(['gear','cyl']).size().plot.barh()
my_colors = list(['b', 'r', 'g', 'y', 'k']), None, len(df))
